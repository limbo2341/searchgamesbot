    ...
    async with async_session_maker() as session:
        ...
        if created or not user.language:
            ...
            await message.answer(
                ...
                reply_markup=main_menu_keyboard(user.language, is_admin=is_admin),
                parse_mode="HTML",
            )
...
async def admin_button_handler(message: Message, state: FSMContext):
    ...
    await message.answer("Admin Panel - Welcome!", reply_markup=admin_main_keyboard())


@router.message(F.text.in_(ALL_BACK_TEXTS))
async def universal_back(message: Message, state: FSMContext):
    """Universal back/main menu handler for all languages."""
    await state.clear()
    user_id = message.from_user.id
    is_admin = user_id in settings.admin_ids
    async with async_session_maker() as session:
